...
public:
  ...
  BP (parameters param) : param(param) {}
  ...
  void process (uint8_t* I1,uint8_t* I2,float* D1,float* D2,const int32_t* dims, IplImage* preL=NULL, IplImage* preD=NULL);
  int Get_addr(int width,int x, int y){return y*width+x;}
  int Census_Cost(uint8_t* Left,uint8_t* Right,int x,int y,int d,int win_size,const int32_t* dims); 
  inline int Box_Cost(uint8_t* Left,uint8_t* Right,int x,int y,int d,int win_size,const int32_t* dims); 
  int ASW_Cost(uint8_t* Left,uint8_t* Right,int x,int y,int d,int win_size,const int32_t* dims); 

  void  BP_process(uint8_t* Left,uint8_t* Right,float* D1_,float* D2_,const int32_t* dims);
  void  BP_process_Overlap(uint8_t* Left,uint8_t* Right,float* D1_,float* D2_,const int32_t* dims,bool For_Right);
  void  Message_Update(float *Message_A,float *Message_B,float *Message_C,float *cost,float *Message_out,int lamda,float Weight_A,float Weight_B,float Weight_C);
  int   Message_Determine(float *Message_A,float *Message_B,float *Message_C,float *Message_D,float *cost,float Weight_A,float Weight_B,float Weight_C,float Weight_D);
  float Message_Weight(uint8_t* Left,int ref_x,int ref_y,int tar_x,int tar_y,const int32_t* dims);
  void  LRC(float* D1_,float* D2_,const int32_t* dims);
...
private:
  ...
  float *M_Cost,*M_Up,*M_Down,*M_Left,*M_Right;
  float *B_Up,*B_Down,*B_Left,*B_Right;
  float *Message_A,*Message_B,*Message_C,*Message_D,*Message_Out,*Message_Cost;
